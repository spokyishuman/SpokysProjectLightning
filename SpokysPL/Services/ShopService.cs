...
namespace SpokysProjectVercel.Services
{
    public class ShopService
    {
        ...
        public async Task<List<ShopItem>> LoadItemsAsync()
        {
            ...
            {
                SyncStatus = "⚠️ No remote URL configured";
                return new();
            }
            ...
            try
            {
                ...
                SyncStatus = $"✅ Cloud — {_items.Count} items";
                return _items.ToList();
            }
            catch
            {
                SyncStatus = "⚠️ Cloud unavailable";
                return new();
            }
        }
        ...
        public async Task<bool> PublishToGithubAsync()
        {
            ...
            try
            {
                var content = JsonConvert.SerializeObject(ToDict(_items), Formatting.Indented);
                ...
                else
                {
                    ...
                    SyncStatus = putResp.StatusCode == System.Net.HttpStatusCode.Forbidden
                        ? "❌ Token rejected (403).\nCreate a classic PAT at github.com/settings/tokens with repo scope."
                        : $"❌ GitHub error: {putResp.StatusCode}";
                    return false;
                }
            }
            catch (Exception ex)
        ...
        {
            var existing = _items.FirstOrDefault(i => i.AppId == item.AppId);
            if (existing != null)
            {
        ...
        {
            _items.RemoveAll(i => i.AppId == appId);
            _ = AutoPublishAsync();
        }
        ...
        private static List<ShopItem> ParseItems(string json)
        {
            ...
            foreach (var entry in obj)
            {
                ...
                items.Add(new ShopItem
                {
                    ...
                    ItemType = jo["tipo"]?.Value<string>() ?? "Steam Game",
                    Description = jo["descripcion"]?.Value<string>() ?? "",
                    CustomImageUrl = jo["customImg"]?.Value<string>() ?? "",
                });
            }
            return items;
        }
